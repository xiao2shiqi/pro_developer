...
3. 短停顿可以保证用户交互体验，高吞吐量则可以保证处理器资源的最高效利用，所以关注吞吐量的 Parallel Scavenge 收集器适合在后台运算不需要太多交互的分析任务
4. 常被称为 **吞吐量优先收集器**， 参数 `-XX:UseAdaptiveSizePolicy` 可以自适应调节策略（GC Ergonomics）
